...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const checkFocusAndHide = (container, target, hideCallback) => {
        setTimeout(() => {
            if (!container.contains(document.activeElement) && !target.contains(document.activeElement)) {
                hideCallback();
            }
        }, 0);
    };
    ...
    document.querySelectorAll(".nav-link_cont").forEach(container => {
        ...
        const showDropdown = () => {
            ...
            animateElements(items, { opacity: 1 }, { 
                duration: 0.6, 
                delay: 0.2, 
                stagger: 0.1 
            });
        ...
        const hideDropdown = () => {
            ...
            animateElements(items, { opacity: 0 }, {
                onComplete: () => gsap.set(items, { clearProps: "opacity" })
            });

            animateElements(target, { height: 0 }, {
    ...
    mobileTrigger?.addEventListener("click", () => {
        const animationProps = isMobileOpen 
            ...
            : { opacity: 1, duration: .6, stagger: 0.1, ease: "power1.out" };
